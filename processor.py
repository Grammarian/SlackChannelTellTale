...
class Processor:
    ...
    def remember_channel(self, channel):
        ...
        if self.redis_client:
            return self._remember_channel_redis(channel)
        else:
            return self._remember_channel_non_redis(channel)


    def _remember_channel_redis(self, channel):
        """
    ...
    def _remember_channel_non_redis(self, channel):
        """
        ...
        if channel_id in self._non_redis_cache:
            return True
        self._non_redis_cache[channel_id] = channel.get("created", "0")
        return False
    ...
    def process_channel_event(self, event_type, event_data):
        ...
        if not channel or \
            ...
            "name" not in channel:
            self.logger.error("ignored... event was missing require attributes")
            return
        ...
        if not creator_id:
            ...
            return
        creator_info = self.slack_client.api_call("users.info", user=creator_id)
    ...
    def _send_pretty_notification(self, event_type, channel, creator):
        ...
        self.slack_client.api_call("chat.postMessage", channel=self.target_channel_id, attachments=[fancy_message])
